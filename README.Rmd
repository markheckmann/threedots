...
`{threedots}` allows to modify the dots directly via `modify_dots()`. It is basically a small wrapper around for the `do.call` approach above.
...
You can use the dots component's names inside `modify_dots()` for calculations using the `.()` function.
